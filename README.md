...
- **Shelf capacity is 24.** The shelf can hold at most 24 jewels at once,
  sized to fit the largest scramble cluster. Change the constant
  `SHELF_CAPACITY` at the top of `src/shuffle.lua` to tune it — `level.lua`
  reads the value from there, so there is exactly one canonical source of
  truth. If you try to park a cluster that wouldn't fit, the deposit is
  rejected, your hover state is preserved, and a brief "Shelf full!" flash
  appears over the shelf.
- **Solvability.** Puzzles are generated by a region-growing scrambler
  that carves the grid into 8-connected spatial regions of 8-24 cells
  and fills each region with one "wrong" jewel color — chunky lifts
  instead of per-cell confetti. Each candidate is validated by a cheap
  greedy solver given the current shelf cap; if the greedy solver can't
  find a solution, we retry. Region-based construction preserves the
  color multiset by design, so the first attempt is almost always
  accepted.
